...
const generateHTML = (
  ...
  let options = opts.swaggerOptions || {};
  let customCss = opts.customCss;
  let customJs = opts.customJs;
  let customfavIcon = opts.customfavIcon || false;
  let swaggerUrl = opts.swaggerUrl;
  let swaggerUrls = opts.swaggerUrls;
  let isExplorer = opts.explorer || !!swaggerUrls;
  let customSiteTitle = opts.customSiteTitle;
  let customCssUrl = opts.customCssUrl;
  let explorerString = isExplorer
    ? ""
    : ".swagger-ui .topbar .download-url-wrapper { display: none }";
  customCss = explorerString + " " + customCss || explorerString;
  customSiteTitle = customSiteTitle || "Swagger UI";
  let _htmlTplString = opts.htmlTplString || htmlTplString;
  let _jsTplString = opts.jsTplString || jsTplString;

  let favIconString = customfavIcon
    ? '<link rel="icon" href="' + customfavIcon + '" />'
    : favIconHtml;
  let htmlWithCustomCss = _htmlTplString
    .toString()
    .replace("<% customCss %>", customCss);
  let htmlWithFavIcon = htmlWithCustomCss.replace(
    "<% favIconString %>",
    favIconString,
  );
  let htmlWithCustomJs = htmlWithFavIcon.replace(
    "<% customJs %>",
    customJs ? `<script src="${customJs}"></script>` : "",
  );
  let htmlWithCustomCssUrl = htmlWithCustomJs.replace(
    "<% customCssUrl %>",
...
const stringify = (obj: any, prop?: any) => {
